...
public class ViewModel {
    ...
    /**
     * This is a subclass as a eventHandler for "droneAlert".
     */
    private class RaiseDroneAlert implements EventHandler<ActionEvent> {
    ...
    /**
     * This is a subclass as a unified eventHandler for "droneClear".
     */
    private class ClearDroneAlert implements EventHandler<ActionEvent> {
        public void handle(ActionEvent e) {
            for (Terminal terminal : tower.getTerminals()) {
    ...
    /**
     * This is a subclass as a unified eventHandler for "Find Gate for Selected Aircraft".
     */
    private class GateHandler implements EventHandler<ActionEvent> {
        public void handle(ActionEvent e) {
            Aircraft aircraft = getSelectedAircraft().get();
            if (aircraft != null) {
                if (aircraft.getTaskList().getCurrentTask().getType()
                        .equals(TaskType.LAND)) {
                    try {
                        Gate gate = tower.findUnoccupiedGate(getSelectedAircraft().get());
                        suitableGateText.set(gate.toString());
    ...
    public void saveAs(Writer tickWriter, Writer aircraftWriter, Writer queuesWriter,
        ...
        try (BufferedWriter tick = new BufferedWriter(tickWriter)) {
            tick.write(String.valueOf(tower.getTicksElapsed()));
            tick.flush();
        }
        try (BufferedWriter aircraft = new BufferedWriter(aircraftWriter)) {
            StringJoiner aircraftInfo = new StringJoiner(System.lineSeparator());
            aircraftInfo.add(String.valueOf(tower.getAircraft().size()));
            for (Aircraft encoded : tower.getAircraft()) {
                aircraftInfo.add(encoded.encode());
            }
            aircraft.write(aircraftInfo.toString());
        }

        try (BufferedWriter queues = new BufferedWriter(queuesWriter)) {
            StringJoiner queuesInfo = new StringJoiner(System.lineSeparator());
            queuesInfo.add(tower.getTakeoffQueue().encode());
            queuesInfo.add(tower.getLandingQueue().encode());
            queuesInfo.add(String.format("LoadingAircraft:%d", tower.getLoadingAircraft().size()));

            Set<Aircraft> aircraftAtLoad = tower.getLoadingAircraft().keySet();
            StringJoiner loading = new StringJoiner(",");
            for (Aircraft aircraft : aircraftAtLoad) {
                loading.add(
                        String.format("%s:%d",
                                aircraft.getCallsign(),
                                tower.getLoadingAircraft().get(aircraft)
                                )
                );
            }
            queuesInfo.add(loading.toString());
            queues.write(queuesInfo.toString());
            queues.flush();
        }

        try (BufferedWriter terminals = new BufferedWriter(terminalsWithGatesWriter)) {
            StringJoiner terminalsInfo = new StringJoiner(System.lineSeparator());
            terminalsInfo.add(String.valueOf(tower.getTerminals().size()));
            for (Terminal terminal : tower.getTerminals()) {
                terminalsInfo.add(terminal.encode());
            }
            terminals.write(terminalsInfo.toString());
            terminals.flush();
        }
    }
